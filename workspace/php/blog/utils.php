<?php
 function get_db(){//データベースからPHPにアクセスするためのオブジェクト
 	//PDOを介して
	 $db = new PDO('mysql:host=localhost;dbname=blog_l;charset=utf8mb4','root','');
	 $db->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
	 return $db;
 }

 function get_post($id){//idを渡している レコードが帰ってくる
	 $sql = "select * from posts where id = '${id}'";
	 $post = get_db()->query($sql)->fetch();
	 return $post;
 }
 function get_limit(){
 	return 5;
 }
function get_posts($offset){//offsetは記事の制限をかけてる
	//何番目からを取ってくる
 	$limit = get_limit();
 	if($offset < 0){
 		$offset = 0;
 	}
 
 	$sql = "select * from posts where status !='draft'order by created_at desc limit ${limit} offset ${offset}";
 	//"aa${bbb}bb" → 
 	$stmt = get_db()->query($sql);
 	return $stmt; 
}

function link_tag($url,$label,$params){//aタグをechoタグで出力するため
	$qs = "?";
	foreach ($params as $key => $param){

		$qs = "${qs}${key}=${param}&";
	}
	$url = "${url}${qs}";
	$tag = "<a href='${url}'>${label}</a>";
  echo  $tag; 
}

 function get_drafts($offset){//status=draft
 	$limit = get_limit();
 	$sql = "select * from posts where status = 'draft' order by created_at desc limit ${limit} offset ${offset}";
 	return get_db()->query($sql);
 }
 function get_drafts_count(){
 	$sql ="select count(*) as count from posts where status = 'draft'";
 	$result = get_db()->query($sql)->fetch();
 	return $result['count'];
 }
 function get_all_posts(){
 	$sql = "select * from posts order by created_at desc";
 	return get_db()->query($sql);
 }

  function get_posts_count(){
 	$sql = "select count(*) as count from posts where status !='draft'";
 	$result =  get_db()->query($sql)->fetch();
 	return $result['count'];
 }
 function is_valid_image($image_path){
 	return(!empty($image_path) and file_exists($image_path) and !ends_with($image_path));
 }
 function ends_with($str){//　　/で終わってたらtrueが帰ってくる
 	//指定位置から
 	//文字数を抜き出す
 	$end = substr($str, strlen($str) - 1);
 	return $end =='/';
 }


?>